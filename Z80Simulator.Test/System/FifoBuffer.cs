...
{
    /// <summary>
    /// Rotating buffer used only for debugging sessions
    /// </summary>
    ...
    /*
    FifoBuffer<string> lastInstructions = new FifoBuffer<string>(40000);

    private void EndInstruction()
    {
        if (instructionOrigin.OpCode != null)
        {
            if (instructionOrigin.Address >= 0x4000)
            {
                lastInstructions.Add(instructionOrigin.Address + ";" + instructionOrigin.OpCode.InstructionText + ";" + PC + ";" + SP);
            }
        }
        else
        {
            lastInstructions.Add(instructionOrigin.Address + ";" + instructionOrigin.Source + ":" + currentInstruction.Type.OpCodeName + ";" + PC + ";" + SP);                
        }            
        if (PC == 0)
        {
            StringBuilder sbLastInstructions = new StringBuilder();
            foreach (string instrLog in lastInstructions)
            {
                sbLastInstructions.AppendLine(instrLog);
            }
